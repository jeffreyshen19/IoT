...
public class SSLTempMonitor extends Thread {
  ...
  public SSLTempMonitor(Socket s, Socket s1) {
    sock = s;
    sock1 = s1;
  }
    ...
    try {
      while (running || running1) {
        ...
        if (running1) {
          ...
          PrintWriter pw = new PrintWriter(sock1.getOutputStream());
          if (receiving) {
            String data = br.readLine();
            messages.add(data);
            System.out.println(data + " is echoed");
